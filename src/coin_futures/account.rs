...
use crate::coin_futures::rest_model::{AccountBalance, AccountInformation, CanceledOrder, ChangeLeverageResponse,
                                      Order, OrderType, Position, PositionSide, Transaction, WorkingType};
use crate::errors::*;
...
impl CoinFuturesAccount {
    ...
    pub async fn get_order(&self, order: Option<GetOrderRequest>) -> Result<Transaction> {
        self.client
            .get_signed_p("/dapi/v1/order", order, self.recv_window)
            .await
    }
    ...
    pub async fn place_order_test(&self, order: OrderRequest) -> Result<Transaction> {
        self.client
            .post_signed_p("/dapi/v1/order/test", order, self.recv_window)
            .await
    }
